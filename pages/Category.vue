                  ...
                  <SfList class="list">
                    <SfListItem class="list__item">
                      <SfMenuItem :count="cat.count || ''" :label="cat.label">
                        <template #label>
                          <nuxt-link :to="localePath(th.getCatLink(cat))" :class="
        ...
        <div class="products" v-if="!loading">
          ...
          <transition-group v-else appear name="products__slide" tag="div" class="products__list">
            <SfProductCardHorizontal v-e2e="'category-product-card'" v-for="(product, i) in products"
              ...
              " -->
              <!-- :is-in-wishlist="isInWishlist({ product })"   @click:wishlist="
                !isInWishlist({ product })
                  ? addItemToWishlist({ product })
                  : removeProductFromWishlist(product)
              " -->
              <template #configuration>
                <SfProperty v-for="(stdAttribute, i) in productGetters.getStandardAttributes(product)" :key="i"
          ...
          <LazyHydrate on-interaction>
            <SfPagination v-if="!loading" class="products__pagination desktop-only" v-show="pagination.totalPages > 1"
...
export default {
  ...
  setup(props, context) {
    ...
    const addToCart = ({ product, quantity }) => {
      const productId = productGetters.getDefaultVarianId(product);
      addItemToCart({
        product: { productId },
        quantity
      });
